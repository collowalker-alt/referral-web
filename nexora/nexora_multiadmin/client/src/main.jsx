...
function AdminManagement({action}){
...
function AdminApp(){const [admin,setAdmin]=useState(null),[loading,setLoading]=useState(true),[page,setPage]=useState("overview"),[data,setData]=useState({overview:null,users:[],transactions:[],withdrawals:[],packages:[],announcements:[],deposits:[]}),[error,setError]=useState(""),[search,setSearch]=useState(""),[mobile,setMobile]=useState(false),[notice,setNotice]=useState(""),[busy,setBusy]=useState(false),[busyMessage,setBusyMessage]=useState("");
 ...
 const nav=[["overview","Overview",BarChart3],["users","Users",UsersRound]]; const r=admin.role; if(["SUPER_ADMIN","ADMIN","FINANCE_ADMIN"].includes(r)) nav.push(["transactions","Transactions",ReceiptText],["deposits","Wallet deposits",WalletCards],["withdrawals","Withdrawals",HandCoins]); if(["SUPER_ADMIN","ADMIN","FINANCE_ADMIN"].includes(r)) nav.push(["repair","Payment repair",Wrench]); if(["SUPER_ADMIN","ADMIN"].includes(r)) nav.push(["packages","Packages",PackageIcon],["advertising","Advertising",Megaphone],["marketplace","Marketplace",Store]); if(["SUPER_ADMIN","ADMIN","SUPPORT_ADMIN"].includes(r)) nav.push(["announcements","Announcements",Megaphone],["support","Support tickets",LifeBuoy]); if(["SUPER_ADMIN","ADMIN"].includes(r)) nav.push(["activity","Activity log",Activity]); if(r==="SUPER_ADMIN") nav.push(["admins","Admin management",UserCog]);
 ...
 {page==="overview"&&<AdminOverview data={data.overview} onRefresh={()=>load(false)}/>} {page==="users"&&<AdminUsers rows={data.users} search={search} setSearch={setSearch} action={action}/>} {page==="transactions"&&<AdminTransactions rows={data.transactions}/>} {page==="repair"&&<AdminPaymentRepair action={action}/>} {page==="paybillverify"&&<AdminPaybillVerify action={action}/>} {page==="deposits"&&<AdminDepositVerify action={action}/>} {page==="withdrawals"&&<AdminWithdrawals rows={data.withdrawals} action={action}/>} {page==="packages"&&<AdminPackages rows={data.packages} action={action}/>} {page==="advertising"&&<AdminAdvertising products={data.adProducts||[]} submissions={data.adSubmissions||[]} action={action} reload={()=>load(false)}/>} {page==="marketplace"&&<AdminMarketplace products={data.marketProducts||[]} orders={data.marketOrders||[]} action={action}/>} {page==="announcements"&&<AdminAnnouncements action={action}/>} {page==="support"&&<AdminSupportTickets action={action}/>} {page==="activity"&&<AdminActivity/>} {page==="admins"&&admin.role==="SUPER_ADMIN"&&<AdminManagement action={action}/>}</main></div>}
...
function App(){
 ...
 const startPayment=async(_,p)=>{setPhoneModal(null);setMsg("");setError("");try{const d=await api("/payments/paybill/initiate",{method:"POST",body:JSON.stringify({packageId:p.id})});setPayment({method:"paybill",reference:d.reference,package:p,status:d.status||"pending",chargeAmount:Number(d.chargeAmount||p.price),paybillNumber:d.paybillNumber,paybillAccount:d.paybillAccount,paybillName:d.paybillName,instructions:d.instructions||[],message:d.message||""});}catch(e){setError(e.message)}};
 const hasPackage=Boolean(me?.package);
 const purchase=p=>{setMsg("");setError("");if(p?.id)setPhoneModal({package:p,chargeAmount:Math.max(0,Number(p.price)-Number(me.package?.price||0))})};
...
function PhoneModal({data,onCancel,onContinue,onWallet,walletBalance=0}){
 const [err,setErr]=useState(""),[method,setMethod]=useState("paybill"),[busy,setBusy]=useState(false),[walletConfirm,setWalletConfirm]=useState(false);
 const amount=Number(data.chargeAmount||data.package?.price||0),canWallet=walletBalance>=amount&&amount>0;
 const useWallet=async()=>{if(!canWallet)return setErr("Insufficient wallet balance for this plan.");setErr("");setMethod("wallet");setWalletConfirm(true)};
 const confirmWallet=async()=>{if(!canWallet)return setErr("Insufficient wallet balance for this plan.");setErr("");setWalletConfirm(false);setBusy(true);try{await onWallet(data.package)}catch(ex){setErr(ex.message||"Wallet purchase failed");setBusy(false)}};
 const submit=async e=>{e.preventDefault();setErr("");if(method==="wallet")return useWallet();setBusy(true);try{await onContinue(null,data.package)}catch(ex){setErr(ex.message||"Could not start Paybill payment");setBusy(false)}};
 return <div className="modalbackdrop" onClick={onCancel}><div className="modal phonemodal" onClick={e=>e.stopPropagation()}>
  ...
  <form className="phonemodalform" onSubmit={submit}><div className="paymentbody phonebody">
   ...
   <div className="paymethods"><button type="button" className={`paymethod ${method==="paybill"?"active":""}`} disabled={busy} onClick={()=>{setMethod("paybill");setErr("")}}><CreditCard size={18}/><div><b>M-Pesa Paybill</b><span>Pay through Co-op Bank Paybill and submit your confirmation code</span></div></button><button type="button" className={`paymethod ${method==="wallet"?"active":""} ${!canWallet?"disabled":""}`} onClick={useWallet} disabled={!canWallet||busy}><WalletCards size={18}/><div><b>Use wallet balance</b><span>{canWallet?`Pay ${money(amount)} instantly from your wallet`:`Need ${money(amount)} · you have ${money(walletBalance)}`}</span></div></button></div>
   {method==="paybill"&&<div className="paybillbox"><div className="paybilldetails"><div><span>Paybill</span><strong>400200</strong></div><div><span>Co-op account</span><strong>Shown after continuing</strong></div><div><span>Amount</span><strong>{money(amount)}</strong></div></div><ol className="paybillsteps"><li>Open <b>M-Pesa → Lipa na M-Pesa → PayBill</b>.</li><li>Enter Paybill number <b>400200</b>.</li><li>Enter the NEXORA Co-op account number shown in the next step.</li><li>Enter exactly <b>{money(amount)}</b> and complete with your M-Pesa PIN.</li><li>Return here and enter the M-Pesa confirmation code.</li></ol><div className="notice"><Info size={15}/> Your payment is manually verified before the plan is activated.</div></div>}
   {method==="wallet"&&walletConfirm&&!busy&&<div className="walletconfirmbox"><div className="walletconfirmicon"><WalletCards size={22}/></div><div><h3>Confirm wallet payment</h3><p className="muted small">Are you sure you want to use your NEXORA wallet balance to pay <strong>{money(amount)}</strong> for the <strong>{data.package?.name}</strong> plan?</p><p className="muted small">Your available balance is <strong>{money(walletBalance)}</strong>.</p></div><div className="walletconfirmactions"><button type="button" className="dangeroutline" onClick={()=>{setWalletConfirm(false);setMethod("paybill");setErr("")}} disabled={busy}>Cancel</button><button type="button" className="primary" onClick={confirmWallet} disabled={busy}>Yes, use my balance</button></div></div>}
...
function PaymentModal({payment,onClose,onReceipt,onSuccess}){
 const [code,setCode]=useState(""),[busy,setBusy]=useState(false),[msg,setMsg]=useState(""),[status,setStatus]=useState(payment.status||"pending");
...
function DepositModal({onClose,load}){
 ...
 return <div className="modalbackdrop" onClick={onClose}><div className="modal depositmodal" onClick={e=>e.stopPropagation()}><div className="modalhead"><div><span className="pill">WALLET DEPOSIT</span><h2>{step==="form"?"Deposit to wallet":step==="done"?"Deposit successful":step==="failed"?"Deposit failed":"Co-op Paybill"}</h2></div><button type="button" className="iconbtn" onClick={onClose}><X size={20}/></button></div><div className="depositbody">
 ...
 {step==="pay"&&payment&&<><div className="paybillbox"><div className="paybilldetails"><div><span>Paybill</span><strong>{payment.paybillNumber}</strong></div><div><span>Account</span><strong>{payment.paybillAccount}</strong></div><div><span>Amount</span><strong>{money(payment.amount)}</strong></div><div><span>Reference</span><strong className="refcode">{payment.reference}</strong></div></div><ol className="paybillsteps"><li>Open M-Pesa → Lipa na M-Pesa → PayBill.</li><li>Enter Paybill <b>{payment.paybillNumber}</b>.</li><li>Enter account <b>{payment.paybillAccount}</b>.</li><li>Enter exactly <b>{money(payment.amount)}</b> and complete with your PIN.</li><li>Return here and submit the M-Pesa confirmation code.</li></ol></div><form onSubmit={submitCode} className="paybillcodeform"><label className="fieldlabel">M-Pesa confirmation code<input required value={code} onChange={e=>setCode(e.target.value)} placeholder="e.g. QH12XXXXXX" autoComplete="one-time-code"/></label>{msg&&<div className="error">{msg}</div>}<button className="primary" disabled={busy}>{busy?"Submitting code…":"Submit confirmation code"}</button></form></>}
