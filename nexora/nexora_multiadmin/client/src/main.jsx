...
function PhoneModal({data,onCancel,onContinue,onWallet,walletBalance=0}){
 ...
 const paybillNumber=paymentConfig.paybillNumber||"400200",paybillAccount=paymentConfig.paybillAccount||"",paybillName=paymentConfig.paybillName||"Co-op Bank";
 const [copied,setCopied]=useState(false);
 const copyAccount=async()=>{if(!paybillAccount)return;try{await navigator.clipboard.writeText(String(paybillAccount));setCopied(true);setTimeout(()=>setCopied(false),2200)}catch{window.prompt("Copy Co-op account number:",String(paybillAccount))}};
