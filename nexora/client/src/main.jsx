...
function AdminApp(){const [admin,setAdmin]=useState(null),[loading,setLoading]=useState(true),[page,setPage]=useState("overview"),[data,setData]=useState({overview:null,users:[],transactions:[],withdrawals:[],packages:[],announcements:[]}),[error,setError]=useState(""),[search,setSearch]=useState(""),[mobile,setMobile]=useState(false),[notice,setNotice]=useState("");
 const load=async(show=true)=>{if(show)setLoading(true);setError("");try{const a=await adminApi("/admin/me");setAdmin(a.admin);const [overview,users,transactions,withdrawals,packages,announcements,adProducts,adSubmissions]=await Promise.all([adminApi("/admin/overview"),adminApi("/admin/users"),adminApi("/admin/transactions"),adminApi("/admin/withdrawals"),adminApi("/admin/packages"),adminApi("/admin/announcements"),adminApi("/admin/ad-products"),adminApi("/admin/ad-submissions")]);setData({overview,users,transactions,withdrawals,packages,announcements,adProducts,adSubmissions});}catch(e){localStorage.removeItem("adminToken");setAdmin(null);if(!String(e.message).toLowerCase().includes("session"))setError(e.message)}finally{setLoading(false)}};
 ...
 const nav=[["overview","Overview",BarChart3],["users","Users",UsersRound],["transactions","Transactions",ReceiptText],["repair","Payment repair",Wrench],["tillverify","Till payments",CheckCircle2],["withdrawals","Withdrawals",HandCoins],["packages","Packages",PackageIcon],["advertising","Advertising",Megaphone],["announcements","Announcements",Megaphone],["support","Support tickets",LifeBuoy],["activity","Activity log",Activity]];
...
function App(){
 ...
 const nav=[
  ["dashboard","Dashboard",LayoutDashboard],["packages","Packages",PackageIcon],["referrals","My Referrals",Users],["analytics","Analytics",BarChart2],["marketing","Marketing Center",Megaphone],["products","Products",Store],["community","Community",Users2],["notifications","Notifications",Bell],["academy","NEXORA Academy",GraduationCap],["leaderboard","Leaderboard",Trophy],["challenges","Challenges",Target],["wallet","Wallet",WalletCards],["transactions","Transactions",History],["support","Help & Support",LifeBuoy],["security","Profile & Security",Shield]
 ];
 ...
 {page==="marketing"&&<Marketing me={me} copy={copy} copyCode={copyCode} copiedKind={copiedKind} share={share}/>} {page==="products"&&<Products me={me}/>} {page==="community"&&<Community announcements={announcements}/>} {page==="notifications"&&<NotificationsCenter me={me} analytics={analytics} tickets={tickets}/>} 
   ...
   <div className="paymentbody phonebody">
    ...
    {tillStep!=="done"&&tillStep!=="code"&&(
     <div className="paymethods">
      <button type="button" className={`paymethod ${method==="till"?"active":""}`} onClick={()=>{setMethod("till");setErr("")}}>
       <MessageCircle size={18}/><div><b>M-Pesa Till</b><span>Pay to our till number, then submit your confirmation code</span></div>
      </button>
