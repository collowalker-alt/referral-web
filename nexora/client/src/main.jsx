...
function ActivityCenter({me,tickets,goPage}){
 const [open,setOpen]=useState(false);
 const items=[];
 ...
 return <div className="modalbackdrop" onClick={onCancel}><div className="modal phonemodal" onClick={e=>e.stopPropagation()}>
  ...
  <form className="phonemodalform" onSubmit={submit}><div className="paymentbody phonebody">
   ...
   {method==="wallet"&&walletConfirm&&!busy&&<div className="walletconfirmbox">
    ...
    <div className="walletconfirmactions"><button type="button" className="secondary" onClick={()=>{setWalletConfirm(false);setMethod("stk");setErr("")}} disabled={busy}>No, go back</button><button type="button" className="primary" onClick={confirmWallet} disabled={busy}>Yes, use my balance</button></div>
   </div>}
