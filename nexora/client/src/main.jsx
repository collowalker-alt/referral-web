...
function PWAInstall({compact=false}){
 ...
 return <>
  <button className={`pwa-install ${compact?'pwa-install-compact':''}`} onClick={openInstaller} title="Install NEXORA as an app"><Download size={16}/><span>{compact?'Install':'Install NEXORA'}</span></button>
  {showHelp&&createPortal(<div className="modalbackdrop installbackdrop" onClick={()=>setShowHelp(false)}><div className="modal pwahelp moderninstall" onClick={e=>e.stopPropagation()}>
...
function PublicLanding({onLogin}){
 ...
 return <div className="publicsite">
  <header className="publicnav"><div className="publicbrand"><img src="/nexora-logo.png"/><div><b>NEXORA</b><small>MARKETPLACE · MEMBER PLATFORM</small></div></div><nav><a href="#features">Features</a><a href="#how">How it works</a><a href="/packages">Packages</a><a href="#faq">FAQ</a></nav><div className="publicactions"><PWAInstall compact/><button className="secondary" onClick={()=>go("login")}><LogIn size={15}/> Sign in</button><button className="primary" onClick={()=>go("register")}><Store size={15}/> Start shopping</button></div></header>
  <main>
...
function Products({me,packages=[],purchase,goPlans}){
 ...
 if(!premium){
  ...
  return <section className="premiumgate">
...
function App(){
 ...
 {page==="dashboard"&&<Dashboard goPage={setPage} me={me} copy={copy} copyCode={copyCode} copiedKind={copiedKind} share={share} goPackages={()=>setPage("packages")} profileStrength={profileStrength} analytics={analytics} tickets={tickets} goSecurity={()=>setPage("security")} load={load}/>} 
   ...
   <div className="paymentbody phonebody">
    ...
    {paybillStep==="done"&&(
     <div className="repairdone" style={{margin:"12px 0"}}>
      ...
      <div><b>Submitted for verification</b><p>{paybillSession?.message||"We will confirm your M-Pesa payment and activate the package once the amount matches."}</p></div>
     </div>
    )}
...
{!hasPackage&&<div className="lockedpanel"><LockKeyhole size={28}/><div><h3>Referral link locked</h3><p className="muted">Your personal referral link unlocks after you activate a membership plan. You can still view network activity below once people join under you.</p></div></div>}
