...
function App(){
 ...
 const nav=[
  ["dashboard","Home",BarChart3],["marketplace","🛒 Marketplace",Store],["referrals","💰 Earn",WalletCards],["products","📣 Advertise",Megaphone],["wallet","Wallet",WalletCards],["packages","Membership",PackageIcon],["transactions","Transactions",History],["marketing","Marketing Center",Megaphone],["academy","🎓 Academy",GraduationCap],["analytics","Analytics",BarChart2],["community","Community",Users2],["notifications","Notifications",Bell],["leaderboard","Leaderboard",Trophy],["challenges","Challenges",Target],["support","Help & Support",LifeBuoy],["security","Profile & Security",Shield]
 ];
 const primaryNavIds=new Set(["dashboard","marketplace","referrals","products","wallet","packages"]);
 ...
 <PendingPaymentBanner transactions={transactions} goPage={setPage}/>{page==="dashboard"&&<Dashboard goPage={setPage} me={me} copy={copy} copyCode={copyCode} copiedKind={copiedKind} share={share} goPackages={()=>setPage("packages")} profileStrength={profileStrength} analytics={analytics} tickets={tickets} goSecurity={()=>setPage("security")} load={load}/>} 
...
function Marketing({me,copy,copyCode,copiedKind,share}){
 ...
 return <section>
  ...
  {!hasPackage&&<div className="lockedpanel"><LockKeyhole size={28}/><div><h3>Referral tools unlock after plan activation</h3><p className="muted">Choose a membership package first. Your personal referral link, QR code and share messages become available once a package is active on your account.</p></div></div>}
  {hasPackage&&<>
