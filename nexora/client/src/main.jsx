  ...
  const pages={
    ...
    privacy:{
      ...
      sections:[
        ...
        ["Payments","Payment processing is handled by Paystack M-Pesa STK Push. NEXORA does not store full card or M-Pesa PIN data."],
...
function AdminUsers({rows,search,setSearch,action}){
 ...
 const changePlan=async actionName=>{if(!planAction||busy)return;const u=planAction.user;const labels={SUSPEND:"suspend",REACTIVATE:"reactivate",DEACTIVATE:"deactivate",REMOVE:"remove"};const verb=labels[actionName]||"update";const ok=await action(()=>adminApi(`/admin/users/${u.id}/plan`,{method:"PATCH",body:JSON.stringify({action:actionName})}),`${u.package?.name||"Plan"} ${verb}ed for ${u.email}`);if(ok){setPlanAction(null);setDetail(null)}};
 return <section className="adminsection">
  ...
  <div className="adminpanel tablepanel"><div className="admintable"><div className="adminrow adminrowhead"><span>User</span><span>Phone</span><span>Plan</span><span>Balance</span><span>Status</span><span>Action</span></div>{filtered.map(u=><div className="adminrow" key={u.id}><div><b>{u.name}</b><small>{u.email}</small><small>Ref: {u.referralCode}</small></div><span>{u.phone}</span><span>{u.package?.name||"—"}{u.package&&<small className={`adminstatus ${String(u.planStatus||"ACTIVE").toLowerCase()}`}>{u.planStatus||"ACTIVE"}</small>}</span><span>{money(u.wallet?.balance)}</span><span><em className={`adminstatus ${u.status.toLowerCase()}`}>{u.status}</em></span><div className="rowactions"><button className="rowaction" onClick={()=>openDetails(u)}><Eye size={14}/> Details</button><button className="rowaction" onClick={()=>openBalance(u)}><WalletCards size={14}/> Balance</button>{u.package&&<button className="rowaction dangeraction" onClick={()=>setPlanAction({user:u})}><Ban size={14}/> Plan</button>}<button className="rowaction" onClick={()=>action(()=>adminApi(`/admin/users/${u.id}/status`,{method:"PATCH",body:JSON.stringify({status:u.status==="ACTIVE"?"SUSPENDED":"ACTIVE"})}),u.status==="ACTIVE"?"User suspended":"User reactivated")}>{u.status==="ACTIVE"?<><Ban size={14}/> Suspend</>:<><UserCheck size={14}/> Activate</>}</button></div></div>)}{!filtered.length&&<div className="adminempty">No users match your search.</div>}</div></div>
...
function PhoneModal({data,onCancel,onContinue,onWallet,walletBalance=0}){
 ...
 return <div className="modalbackdrop" onClick={onCancel}><div className="modal phonemodal" onClick={e=>e.stopPropagation()}>
  ...
  <form className="phonemodalform" onSubmit={submit}><div className="paymentbody phonebody">
   ...
   {method==="wallet"&&walletConfirm&&!busy&&<div className="walletconfirmbox">
    <div className="walletconfirmicon"><WalletCards size={22}/></div>
    <div><h3>Confirm wallet payment</h3><p className="muted small">Are you sure you want to use your NEXORA wallet balance to pay <strong>{money(amount)}</strong> for the <strong>{data.package?.name}</strong> plan?</p><p className="muted small">Your available balance is <strong>{money(walletBalance)}</strong>. This payment will be submitted immediately after you confirm.</p></div>
    <div className="walletconfirmactions"><button type="button" className="dangeroutline" onClick={()=>{setWalletConfirm(false);setMethod("stk");setErr("")}} disabled={busy}>Cancel</button><button type="button" className="primary" onClick={confirmWallet} disabled={busy}>Yes, use my balance</button></div>
   </div>}
...
function PaymentModal({payment,onClose,onCheck,onReceipt}){
 ...
 return <div className="modalbackdrop" onClick={onClose}><div className="modal paymentmodal" onClick={e=>e.stopPropagation()}><div className="modalhead"><div><span className={`pill ${isSuccess?"payment-success":isFailed?"payment-failed":"payment-pending"}`}>{isSuccess?"COMPLETED":isFailed?"FAILED":"WAITING FOR PAYMENT"}</span><h2>{title}</h2></div><button className="iconbtn" onClick={onClose}><X size={20}/></button></div><div className="paymentbody"><div className="paymentstatus"><div className={`paymenticon ${isSuccess?"success":isFailed?"failed":"pending"}`}>{isSuccess?<CheckCircle2 size={28}/>:isFailed?<X size={28}/>:<RefreshCw size={28}/>}</div><div><b>{text}</b><p className="muted">{isSuccess?"Payment confirmed by NEXORA.":isFailed?(payment.display_text||"Please try the payment again."):(payment.display_text||"Please enter your M-Pesa PIN on your phone to approve the payment.")}</p></div></div><div className="paymentdetails"><div><span>Plan</span><strong>{payment.package?.name}</strong></div><div><span>Amount</span><strong>{money(payment.chargeAmount||payment.package?.price)}</strong></div><div><span>M-Pesa number</span><strong>{payment.phone}</strong></div><div><span>Reference</span><strong>{payment.reference}</strong></div></div>{isPending&&<div className="paymentwait"><div className="paymentwaittitle"><span className="live-dot"></span><strong>Waiting for confirmation</strong></div><p className="muted small">We will keep checking automatically. Paystack confirms the payment automatically. NEXORA may verify the transaction while it is still pending.</p></div>}{payment.message&&<p className="muted small">{payment.message}</p>}</div><div className="modalfoot paymentfoot">{isPending&&<button className="secondary" onClick={onCheck}><RefreshCw size={16}/> Check payment status</button>}<>{isSuccess&&<button className="secondary" onClick={()=>onReceipt?.(payment)}><ReceiptText size={16}/> View receipt</button>}<button className="primary paymentclose" onClick={onClose}>{isSuccess?"Go to dashboard":isFailed?"Close":"Close for now"}</button></></div></div></div>}
...
function DepositModal({onClose,load,defaultPhone=""}){
 ...
 const poll=async reference=>{let tries=0;const check=async()=>{if(tries>=18)return;tries++;try{const v=await api(`/wallet/deposit/status/${reference}`);setPayment(x=>x?{...x,status:v.status||"pending",display_text:v.display_text||x.display_text,message:v.message||x.message}:x);if(v.status==="success"){setStep("done");await load(false);return}if(v.status==="failed"){setStep("failed");return}}catch(e){setMsg(e.message)}if(tries<18)setTimeout(check,10000)};setTimeout(check,5000)};
 ...
 return <div className="modalbackdrop" onClick={onClose}><div className="modal depositmodal" onClick={e=>e.stopPropagation()}><div className="modalhead"><div><span className="pill">WALLET DEPOSIT</span><h2>{step==="form"?"Deposit to wallet":step==="done"?"Deposit successful":step==="failed"?"Deposit failed":"M-Pesa STK Push"}</h2></div><button type="button" className="iconbtn" onClick={onClose}><X size={20}/></button></div><div className="depositbody">
  ...
  {step==="waiting"&&payment&&<div className="paymentbody"><div className="paymentstatus"><div className="paymenticon pending"><RefreshCw size={28}/></div><div><b>Check your phone for the M-Pesa PIN prompt</b><p className="muted">Approve the payment on {payment.phone}. NEXORA is checking Paystack for the final status.</p></div></div><div className="paymentdetails"><div><span>Amount</span><strong>{money(payment.amount)}</strong></div><div><span>M-Pesa number</span><strong>{payment.phone}</strong></div><div><span>Reference</span><strong>{payment.reference}</strong></div></div>{msg&&<div className="error">{msg}</div>}<button className="secondary" disabled={busy} onClick={checkNow}><RefreshCw size={16}/> Check payment status</button></div>}
