...
function PublicLanding({onLogin}){
 ...
 return <div className="publicsite">
  ...
  <main>
   ...
   <section id="features" className="publicsection"><div className="sectionhead"><div><span className="pill">PLATFORM FEATURES</span><h2>More than a referral link</h2><p className="muted">A useful member experience gives you reasons to return, learn and manage your account.</p></div></div><div className="featuregrid">{[[BarChart2,"Analytics","Understand your network activity, conversions and recorded commissions."],[Megaphone,"Marketing Center","Your personal link, QR sharing and ready-to-edit outreach templates."],[GraduationCap,"NEXORA Academy","Practical lessons on platform use, ethical marketing and account safety."],[Trophy,"Achievements","Track milestones and activity without promising or guaranteeing income."],[LifeBuoy,"Member Support","Create support tickets and keep replies in one place."],[Shield,"Security Center","Manage your profile and keep your account information current."]].map(([I,t,d])=><div className="featurecard" key={t}><div className="featureicon"><I size={22}/></div><h3>{t}</h3><p>{d}</p></div>)}</div></section>
   <section id="how" className="publicsection darksection"><div className="sectionhead"><div><span className="pill">SIMPLE FLOW</span><h2>How NEXORA works</h2></div></div><div className="steps">{[["01","Create","Create your account and complete your profile."],["02","Explore","Review membership benefits, learning tools and your workspace."],["03","Connect","Use your personal referral tools to share NEXORA with people who genuinely want to learn about it."],["04","Track","Follow activity, transactions, support requests and achievements from your dashboard."]].map(([n,t,d])=><div className="step" key={n}><b>{n}</b><h3>{t}</h3><p>{d}</p></div>)}</div></section>
...
function AdminApp(){const [admin,setAdmin]=useState(null),[loading,setLoading]=useState(true),[page,setPage]=useState("overview"),[data,setData]=useState({overview:null,users:[],transactions:[],withdrawals:[],packages:[],announcements:[]}),[error,setError]=useState(""),[search,setSearch]=useState(""),[mobile,setMobile]=useState(false),[notice,setNotice]=useState("");
 const load=async(show=true)=>{if(show)setLoading(true);setError("");try{try{setPayMethods(await api("/payments/methods"))}catch{};const a=await adminApi("/admin/me");setAdmin(a.admin);const [overview,users,transactions,withdrawals,packages,announcements]=await Promise.all([adminApi("/admin/overview"),adminApi("/admin/users"),adminApi("/admin/transactions"),adminApi("/admin/withdrawals"),adminApi("/admin/packages"),adminApi("/admin/announcements")]);setData({overview,users,transactions,withdrawals,packages,announcements});}catch(e){localStorage.removeItem("adminToken");setAdmin(null);if(!String(e.message).toLowerCase().includes("session"))setError(e.message)}finally{setLoading(false)}};
 ...
 const nav=[["overview","Overview",BarChart3],["users","Users",UsersRound],["transactions","Transactions",ReceiptText],["repair","Payment repair",Wrench],["tillverify","Till payments",CheckCircle2],["withdrawals","Withdrawals",HandCoins],["packages","Packages",PackageIcon],["announcements","Announcements",Megaphone],["support","Support tickets",LifeBuoy],["activity","Activity log",Activity]];
...
function App(){
 ...
 if(["/terms","/privacy","/membership"].includes(window.location.pathname)) return <LegalPage type={window.location.pathname.slice(1)}/>;
 if(window.location.pathname==="/packages")||window.location.pathname==="/packages/") return <PublicPackagesPage/>;
 ...
 const nav=[
  ["dashboard","Dashboard",LayoutDashboard],["packages","Packages",PackageIcon],["referrals","My Referrals",Users],["analytics","Analytics",BarChart2],["marketing","Marketing Center",Megaphone],["community","Community",Users2],["notifications","Notifications",Bell],["academy","NEXORA Academy",GraduationCap],["leaderboard","Leaderboard",Trophy],["challenges","Challenges",Target],["wallet","Wallet",WalletCards],["transactions","Transactions",History],["support","Help & Support",LifeBuoy],["security","Profile & Security",Shield]
 ];
 ...
 const copy=async()=>{
  ...
  try{await navigator.clipboard.writeText(link);setMsg("Referral link copied.")}catch{window.prompt("Copy your referral link:",link)}
 };
 const share=async()=>{
  if(!hasPackage){setError("Purchase a package first to unlock your referral link.");return;}
  const link=`${location.origin}/?ref=${me.user.referralCode}`;
 ...
 {page==="dashboard"&&<Dashboard goPage={setPage} me={me} copy={copy} share={share} goPackages={()=>setPage("packages")} profileStrength={profileStrength} analytics={analytics} tickets={tickets} goSecurity={()=>setPage("security")}/>} 
 {page==="packages"&&<Packages packages={packages} current={me.package} purchase={purchase} reload={()=>load(false)}/>} 
 {page==="referrals"&&<><Referrals data={referrals} earnings={earnings} me={me} copy={copy} share={share}/><ReferralTree data={referrals} me={me}/></>} 
 {page==="analytics"&&<Analytics data={analytics} earnings={earnings} referrals={referrals}/>} 
 {page==="marketing"&&<Marketing me={me} copy={copy} share={share}/>} {page==="community"&&<Community announcements={announcements}/>} {page==="notifications"&&<NotificationsCenter me={me} analytics={analytics} tickets={tickets}/>} 
...
function Marketing({me,copy,share}){
 const hasPackage=Boolean(me?.package);
 const link=`${location.origin}/?ref=${me.user.referralCode}`;
 const scripts=[
 ...
 const copyText=async(t)=>{try{await navigator.clipboard.writeText(t); }catch{ window.prompt("Copy message:",t); }};
 return <section>
...
function Academy(){
 ...
 const lessons=[
  ...
  {title:'Understanding membership packages',level:'FOUNDATIONS',time:'7 min',text:'Understand package levels, upgrades, configured benefits and earning eligibility.',sections:[['Package levels','NEXORA has Starter, Growth, Pro, Elite and Premium levels. Each level can have its own configured benefits, price and commission settings.'],['Progressive earning access','Starter is eligible for Starter purchases; Growth for Starter + Growth; Pro for Starter + Growth + Pro; Elite for Starter + Growth + Pro + Elite; Premium for all five, subject to the platform rules.'],['Upgrading','When upgrading, NEXORA charges the difference between your current package price and the target package price. Review the displayed amount before confirming.']],takeaway:'Always check the current package details shown in NEXORA before making a membership decision.'},
