...
function Instructions({onClose}){return <div className="modalbackdrop" onClick={onClose}><div className="modal instructions" onClick={e=>e.stopPropagation()}><div className="modalhead"><div><span className="pill">NEXORA GUIDE</span><h2>How to use NEXORA</h2></div><button className="iconbtn" onClick={onClose}><X size={20}/></button></div><div className="instructionbody"><div><b>1. Create your account</b><p>Register with your name, email, password and valid Kenyan M-Pesa phone number. Keep your login details private.</p></div><div><b>2. Explore your workspace</b><p>Dashboard shows your account, package, referral activity, wallet, notifications and recent activity.</p></div><div><b>3. Choose or upgrade a package</b><p>Review package prices and benefits before purchasing. Upgrades charge only the displayed price difference.</p></div><div><b>4. Understand the two earning paths</b><p>Starter, Growth, Pro and Elite members earn through qualifying referrals according to the package rules. Premium members can also access the Products & Advertising area and may earn from approved advertising campaigns.</p></div><div><b>5. Advertising is Premium-only</b><p>Products are not available to non-Premium members. Premium members can select available products, promote them on WhatsApp Status, TikTok, Instagram, X or another approved platform, then submit the public post link, upload the exact post creative you published, and provide the platform performance figures.</p></div><div><b>6. Advertising payouts</b><p>Approved advertising earnings are processed weekly on Friday. The amount is based on verified views and engagements and the rates configured for the specific campaign. Submission does not guarantee approval or a particular payout.</p></div><div><b>7. Grow your referral network</b><p>Use My Referrals and Marketing Center to share your personal referral link. Referral commissions are recorded only when the qualifying referral purchase is completed and your package is eligible.</p></div><div><b>8. Use analytics, challenges and achievements</b><p>Analytics shows recorded activity. Challenges and achievements track activity milestones; neither guarantees future earnings.</p></div><div><b>9. Learn in NEXORA Academy</b><p>Use Academy to learn packages, referrals, advertising, ethical promotion, analytics, payments, security and responsible communication.</p></div><div><b>10. Wallet and payments</b><p>Wallet shows available, pending and withdrawn amounts. Transactions keeps your payment history. Never share your password, M-Pesa PIN or security codes.</p></div><div><b>11. Get help</b><p>Open Help & Support for a ticket or use WhatsApp Support. For payment or advertising questions, include the relevant reference or campaign information, but never send your PIN.</p></div><div><b>12. Use the NEXORA Marketplace</b><p>Marketplace lets members browse products, search by category and location, add items to a cart, provide delivery details and place orders. Members can also list products with photos, descriptions, prices, stock and location. Keep listings accurate and use safe payment and delivery practices.</p></div><div><b>13. Install NEXORA as an app</b><p>Use the Install NEXORA button when supported. On iPhone/iPad, open NEXORA in Safari and choose Add to Home Screen.</p></div></div><div className="modalfoot"><a className="secondary supportinline" href="https://wa.me/254703265774" target="_blank" rel="noreferrer"><MessageCircle size={17}/> WhatsApp Support <ExternalLink size={14}/></a></div></div></div>}
...
function PublicLanding({onLogin}){
 ...
 return <div className="publicsite">
  <header className="publicnav"><div className="publicbrand"><img src="/nexora-logo.png"/><div><b>NEXORA</b><small>MEMBER PLATFORM</small></div></div><nav><a href="#features">Features</a><a href="#how">How it works</a><a href="/packages">Packages</a><a href="#faq">FAQ</a></nav><div className="publicactions"><PWAInstall compact/><button className="secondary" onClick={()=>go("login")}>Log in</button><button className="primary" onClick={()=>go("register")}>Join NEXORA</button></div></header>
  <main>
   <section className="publichero"><div className="publicheroCopy"><span className="pill">NEXORA · YOUR MEMBER WORKSPACE</span><h1>Connect. Grow. Learn. <span>Build.</span></h1><p>NEXORA brings membership tools, referral analytics, learning resources, support and a personal workspace together in one platform.</p><div className="heroactions"><button className="primary" onClick={()=>go("register")}>Create your account <ArrowUpRight size={17}/></button><button className="secondary" onClick={()=>document.getElementById("how")?.scrollIntoView({behavior:"smooth"})}>See how it works</button></div><div className="publictrust"><span><ShieldCheck size={16}/> Account-focused workspace</span><span><BarChart3 size={16}/> Transparent activity tracking</span><span><LifeBuoy size={16}/> Member support</span></div></div><div className="publicheroCard"><img src="/nexora-logo.png"/><div className="floatingmetric"><small>MEMBER HUB</small><b>One place for your NEXORA journey</b><span>Dashboard · Learning · Analytics · Support</span></div></div></section>
   <section id="features" className="publicsection"><div className="sectionhead"><div><span className="pill">PLATFORM FEATURES</span><h2>More than a referral link</h2><p className="muted">A useful member experience gives you reasons to return, learn and manage your account.</p></div></div><div className="featuregrid">{[[Store,"Premium Advertising","After activating Premium, eligible members can promote approved products and submit verified performance for Friday payout review."],[BarChart2,"Analytics","Understand your network activity, conversions and recorded commissions."],[Megaphone,"Marketing Center","Your personal link, QR sharing and ready-to-edit outreach templates."],[GraduationCap,"NEXORA Academy","Practical lessons on platform use, ethical marketing and account safety."],[Trophy,"Achievements","Track milestones and activity without promising or guaranteeing income."],[LifeBuoy,"Member Support","Create support tickets and keep replies in one place."],[Shield,"Security Center","Manage your profile and keep your account information current."]].map(([I,t,d])=><div className="featurecard" key={t}><div className="featureicon"><I size={22}/></div><h3>{t}</h3><p>{d}</p></div>)}</div></section>
...
function PublicPackagesPage(){
 ...
 useEffect(()=>{
  (async()=>{
   ...
   }catch(e){setErr(e.message||"Unable to load packages");}
 ...
 return <div className="publicsite publicpackagespage">
  <header className="publicnav"><div className="publicbrand"><img src="/nexora-logo.png" alt=""/><div><b>NEXORA</b><small>MEMBER PLATFORM</small></div></div>
   <nav><a href="/">Home</a><a href="/packages">Packages</a><a href="/#faq">FAQ</a></nav>
  ...
  <main className="publicpackagesmain">
   <div className="sectionhead"><div><span className="pill">MEMBERSHIP</span><h1>Compare NEXORA packages</h1><p className="muted">Transparent pricing and benefits. Commissions are recorded only when qualifying referral purchases complete — never guaranteed income.</p></div></div>
    ...
    <div className="packages">
     {sorted.map((p,i)=>(
      <div className={`pkg ${p.popular?"featured":""}`} key={p.id}>
       ...
       <p className="muted small">{p.description||"Membership package with referral tools and platform access."}</p>
       ...
       <a className="primary" href="/?join=1">Join to choose this package</a>
      </div>
     ))}
     {!sorted.length&&<p className="muted">No packages available right now.</p>}
    </div>
   )}
   <div className="earningsnotice" style={{marginTop:24}}><div><strong>Important</strong><p>Package levels define which purchases you may be eligible to earn from. NEXORA does not guarantee income. Review Terms and Membership Rules before paying.</p></div></div>
  </main>
  <footer className="publicfooter"><div><b>NEXORA.</b><p>Connect. Grow. Learn. Build.</p></div><div className="footerlinks"><a href="/terms">Terms</a><a href="/privacy">Privacy</a><a href="/membership">Membership rules</a><a href="/packages">Packages</a></div></footer>
 </div>;
}
...
function AdminApp(){const [admin,setAdmin]=useState(null),[loading,setLoading]=useState(true),[page,setPage]=useState("overview"),[data,setData]=useState({overview:null,users:[],transactions:[],withdrawals:[],packages:[],announcements:[],deposits:[]}),[error,setError]=useState(""),[search,setSearch]=useState(""),[mobile,setMobile]=useState(false),[notice,setNotice]=useState("");
 const load=async(show=true)=>{if(show)setLoading(true);setError("");try{const a=await adminApi("/admin/me");setAdmin(a.admin);const [overview,users,transactions,withdrawals,packages,announcements,adProducts,adSubmissions,deposits]=await Promise.all([adminApi("/admin/overview"),adminApi("/admin/users"),adminApi("/admin/transactions"),adminApi("/admin/withdrawals"),adminApi("/admin/packages"),adminApi("/admin/announcements"),adminApi("/admin/ad-products"),adminApi("/admin/ad-submissions"),adminApi("/admin/wallet/deposits")]);setData({overview,users,transactions,withdrawals,packages,announcements,adProducts,adSubmissions,deposits});}catch(e){localStorage.removeItem("adminToken");setAdmin(null);if(!String(e.message).toLowerCase().includes("session"))setError(e.message)}finally{setLoading(false)}};
 ...
 const nav=[["overview","Overview",BarChart3],["users","Users",UsersRound],["transactions","Transactions",ReceiptText],["repair","Payment repair",Wrench],["tillverify","Till payments",CheckCircle2],["deposits","Wallet deposits",WalletCards],["withdrawals","Withdrawals",HandCoins],["packages","Packages",PackageIcon],["advertising","Advertising",Megaphone],["announcements","Announcements",Megaphone],["support","Support tickets",LifeBuoy],["activity","Activity log",Activity]];
 ...
 {page==="overview"&&<AdminOverview data={data.overview} onRefresh={()=>load(false)}/>} {page==="users"&&<AdminUsers rows={data.users} search={search} setSearch={setSearch} action={action}/>} {page==="transactions"&&<AdminTransactions rows={data.transactions}/>} {page==="repair"&&<AdminPaymentRepair action={action}/>} {page==="tillverify"&&<AdminTillVerify action={action}/>} {page==="deposits"&&<AdminDepositVerify action={action}/>} {page==="withdrawals"&&<AdminWithdrawals rows={data.withdrawals} action={action}/>} {page==="packages"&&<AdminPackages rows={data.packages} action={action}/>} {page==="advertising"&&<AdminAdvertising products={data.adProducts||[]} submissions={data.adSubmissions||[]} action={action} reload={()=>load(false)}/>} {page==="announcements"&&<AdminAnnouncements action={action}/>} {page==="support"&&<AdminSupportTickets action={action}/>} {page==="activity"&&<AdminActivity/>}</main></div>}
...
function AdminTillVerify({action}){
 ...
 const load=async()=>{
  ...
  try{setRows(await adminApi("/admin/payments/pending-till"))}
  catch(e){setErr(e.message)}
  finally{setLoading(false)};
 };
 useEffect(()=>{load()},[]);
 const verify=async(row, act)=>{
  ...
  try{
   const d=await adminApi("/admin/payments/verify-till",{method:"POST",body:JSON.stringify({reference:row.reference,action:act,confirmedAmount:row.amount,note})});
 ...
 return <section className="adminsection">
  <div className="adminsectionhead"><div><span className="pill">TILL VERIFICATION</span><h1>Confirm M-Pesa till payments</h1><p className="muted">Only approve after you confirm the M-Pesa SMS/till statement shows the same amount and confirmation code.</p></div>
   <button className="secondary" onClick={load}><RefreshCw size={16}/> Refresh</button></div>
  {err&&<div className="error">{err}</div>}
  {loading&&<p className="muted">Loading…</p>}
  <div className="table admin-till-table">
   {rows.map(r=>(
    <div className="row tillrow" key={r.id}>
   ...
   {!loading&&!rows.length&&<p className="muted">No pending till payments.</p>}
  </div>
 </section>;
}
...
function ReferralTree({data,me}){const direct=data?.direct||[];const level2=data?.level2||[];return <div className="panel trepanel"><div className="paneltitle"><h3><Users2 size={18}/> Network map</h3><span>{direct.length} direct · {level2.length} level 2</span></div><div className="tree"><div className="treeperson root"><div className="avatar">{me.user.name?.[0]?.toUpperCase()||"N"}</div><b>{me.user.name?.split(" ")[0]}</b><small>Your account</small></div><div className="treebranch">{direct.slice(0,8).map((x,i)=><div className="treecolumn" key={x.id}><div className="treeperson"><div className="avatar">{x.name?.[0]?.toUpperCase()||"M"}</div><b>{x.name?.split(" ")[0]}</b><small>{x.package?.name||"No package"}</small></div><div className="treechildren">{level2.filter(y=>y.referredById===x.id).slice(0,4).map(y=><div className="treeperson mini" key={y.id}><div className="avatar">{y.name?.[0]?.toUpperCase()||"M"}</div><b>{y.name?.split(" ")[0]}</b></div>)}</div></div>)}</div>{!direct.length&&<p className="muted">Your network map will appear here as your direct referrals join.</p>}</div></div>}
function ProfileCard({me,strength,goSecurity}){return <div className="panel profilecard"><div className="profileavatar">{me.user.name?.[0]?.toUpperCase()||"N"}</div><div className="profileinfo"><span className="pill">MEMBER PROFILE</span><h2>{me.user.name}</h2><p>{me.user.email}</p><div className="profilemeta"><span>{me.package?.name||"No package"}</span><span>Code: {me.user.referralCode}</span><span>Member since {new Date(me.user.createdAt).toLocaleDateString()}</span></div><div className="profilemeter"><b>Profile completeness · {strength}%</b><div className="progress"><i style={{width:`${strength}%`}}/></div></div><button className="secondary" onClick={goSecurity}><UserCog size={15}/> Edit profile</button></div></div>}
function Products({me}){
 ...
 if(!premium)return <section><div className="sectionhead"><div><span className="pill">PREMIUM MEMBER AREA</span><h1>Products & Advertising</h1><p className="muted">The NEXORA advertising marketplace is available only after activating Premium.</p></div></div><div className="lockedpanel productlock"><Store size={30}/><div><h3>Premium required</h3><p className="muted">Starter, Growth, Pro and Elite members continue with referral earning. Premium members unlock approved advertising products and may earn from verified campaign performance.</p></div></div><div className="notice"><CalendarCheck2 size={16}/> Approved advertising payouts are processed weekly on Friday and depend on verified views and engagements. No fixed income is guaranteed.</div></section>;
...
function Marketplace({me}){
 const [tab,setTab]=useState("shop"),[products,setProducts]=useState([]),[mine,setMine]=useState([]),[orders,setOrders]=useState({buying:[],selling:[]}),[cart,setCart]=useState(()=>{try{return JSON.parse(localStorage.getItem("nexora-cart")||"[]")}catch{return[]}}),[q,setQ]=useState(""),[category,setCategory]=useState("All"),[location,setLocation]=useState(""),[selected,setSelected]=useState(null),[notice,setNotice]=useState(""),[error,setError]=useState(""),[busy,setBusy]=useState(false),[form,setForm]=useState({title:"",description:"",price:"",stock:"1",category:"Other",location:"",phone:me.user.phone||"",images:[]}),[checkout,setCheckout]=useState({deliveryName:me.user.name||"",deliveryPhone:me.user.phone||"",deliveryAddress:"",deliveryNotes:"",paymentMethod:"CASH_ON_DELIVERY"});
 ...
 useEffect(()=>{loadProducts()},[category]); useEffect(()=>{if(tab==="sell")loadMine();if(tab==="orders")loadOrders()},[tab]);
...
function App(){
 ...
 const nav=[
  ["dashboard","Dashboard",LayoutDashboard],["packages","Packages",PackageIcon],["referrals","My Referrals",Users],["analytics","Analytics",BarChart2],["marketing","Marketing Center",Megaphone],["marketplace","Marketplace",Store],["products","Products & Ads",Megaphone],["community","Community",Users2],["notifications","Notifications",Bell],["academy","NEXORA Academy",GraduationCap],["leaderboard","Leaderboard",Trophy],["challenges","Challenges",Target],["wallet","Wallet",WalletCards],["transactions","Transactions",History],["support","Help & Support",LifeBuoy],["security","Profile & Security",Shield]
 ];
 ...
 const tillInitiate=async(p)=>{
  const d=await api("/payments/till/initiate",{method:"POST",body:JSON.stringify({packageId:p.id})});
  return d;
 };
 const tillSubmitCode=async(reference,mpesaCode)=>{
  const d=await api("/payments/till/submit-code",{method:"POST",body:JSON.stringify({reference,mpesaCode})});
  await load(false);
 ...
 const walletPurchase=async(p)=>{
  ...
  setPhoneModal(null);
  await load(false);
  setMsg(d.message||"Package activated using your wallet balance.");
 };
 ...
 const copy=async()=>{
  if(!hasPackage){setError("Purchase a package first to unlock your referral link.");return;}
 ...
 const copyCode=async()=>{
  if(!hasPackage){setError("Purchase a package first to unlock your referral code.");return;}
 ...
 return <div className="app">{mobile&&<button className="navoverlay" aria-label="Close menu" onClick={()=>setMobile(false)}/>}<aside className={mobile?"open":""}><div className="membernavbrand"><img src="/nexora-logo.png"/><div><b>NEXORA</b><small>MEMBER PLATFORM</small></div><button className="mobileclose" onClick={()=>setMobile(false)} aria-label="Close menu"><X size={19}/></button></div><div className="membernavscroll">{nav.map(([id,t,I])=><button className={page===id?"active":""} onClick={()=>{setPage(id);setMobile(false);setMsg("")}} key={id}><I size={18}/>{t}{id==="support" && tickets.some(x=>x.status==="OPEN") && <span className="navbadge">!</span>}</button>)}<button onClick={()=>{setInstructions(true);setMobile(false)}}><BookOpen size={18}/>Instructions</button><button onClick={()=>{setPage("security");setMobile(false)}}><UserCog size={18}/>Profile & Security</button><button className="logoutbtn" onClick={logout}><LogOut size={18}/>Logout</button></div></aside><main><header><button className="mobilemenu" onClick={()=>setMobile(!mobile)} aria-label={mobile?"Close menu":"Open menu"} title={mobile?"Close navigation":"Open navigation"}>{mobile?<X size={22}/>:<span className="hamburgerglyph" aria-hidden="true">☰</span>}</button><div className="memberpagetitle"><b>{nav.find(x=>x[0]===page)?.[1]||"Dashboard"}</b><div className="muted small">Your NEXORA member workspace</div></div><div className="memberheaderbrand"><img src="/nexora-logo.png"/><PWAInstall compact/><button className="avatar avatarbtn" onClick={()=>setMobile(true)} aria-label="Open account menu">{me.user.name?.[0]?.toUpperCase()||"N"}</button></div></header>{error&&<div className="error topmsg"><span>{error}</span><button onClick={()=>load(false)}><RefreshCw size={15}/> Retry</button></div>}{msg&&<div className="notice topmsg">{msg}</div>}
 {page==="dashboard"&&<Dashboard goPage={setPage} me={me} copy={copy} copyCode={copyCode} copiedKind={copiedKind} share={share} goPackages={()=>setPage("packages")} profileStrength={profileStrength} analytics={analytics} tickets={tickets} goSecurity={()=>setPage("security")}/>} 
 {page==="packages"&&<Packages packages={packages} current={me.package} purchase={purchase} reload={()=>load(false)}/>} 
 {page==="referrals"&&<><Referrals data={referrals} earnings={earnings} me={me} copy={copy} copyCode={copyCode} copiedKind={copiedKind} share={share}/><ReferralTree data={referrals} me={me}/></>} 
 {page==="analytics"&&<Analytics data={analytics} earnings={earnings} referrals={referrals}/>} 
 {page==="marketing"&&<Marketing me={me} copy={copy} copyCode={copyCode} copiedKind={copiedKind} share={share}/>} {page==="marketplace"&&<Marketplace me={me}/>} {page==="products"&&<Products me={me}/>} {page==="community"&&<Community announcements={announcements}/>} {page==="notifications"&&<NotificationsCenter me={me} analytics={analytics} tickets={tickets}/>} 
...
function OnboardingChecklist({me,goPackages,goPage}){
 ...
 const defaults={profile:Boolean(me?.user?.phone&&me?.user?.name),package:Boolean(me?.package),shared:false,academy:false};
 const [done,setDone]=useState(()=>{
  try{return {...defaults,...JSON.parse(localStorage.getItem(storageKey)||"{}")};}catch{return defaults;}
 });
 useEffect(()=>{
  setDone(d=>({...d,profile:Boolean(me?.user?.phone&&me?.user?.name),package:Boolean(me?.package)}));
 ...
 const items=[
  {id:"profile",label:"Complete your profile details",ok:done.profile,action:()=>goPage&&goPage("security")},
  {id:"package",label:"Choose or activate a package",ok:done.package,action:goPackages},
  {id:"shared",label:"Share your referral link (after package)",ok:done.shared,action:()=>{if(!me?.package){goPackages&&goPackages();return;}goPage&&goPage("marketing");setDone(x=>({...x,shared:true}));}},
  {id:"academy",label:"Open one Academy lesson",ok:done.academy,action:()=>{goPage&&goPage("academy");setDone(x=>({...x,academy:true}));}}
 ];
 const completed=items.filter(x=>x.ok).length;
 if(completed>=items.length) return null;
 return <div className="panel onboardingpanel">
  <div className="paneltitle"><h3><Sparkles size={17}/> Getting started</h3><span>{completed}/{items.length} done</span></div>
...
function Marketing({me,copy,copyCode,copiedKind,share}){
 ...
 const scripts=[
  ...
  ["Curiosity","NEXORA is a member workspace with packages, referrals, Academy lessons and a wallet. Want me to walk you through it before you decide?"],
  ["Value-first","Before anything else, look at the packages and the rules. NEXORA does not promise income — it gives tools. Here is my invite link if you want to explore."],
  ["Follow-up","Did you get a chance to open NEXORA? I can explain the packages and how referrals are recorded so there is no confusion."]
 ];
  ...
  {hasPackage&&<>
   <div className="marketinggrid">
    ...
    <div className="panel"><Sparkles size={24}/><h3>Ready-to-share messages</h3>
     ...
     <div className="template"><b>Honest invite</b><p>I'm on NEXORA. It is a membership platform with packages, referrals and Academy lessons. No guaranteed income — review the packages first: {link}</p><button className="secondary" onClick={()=>copyText(`I'm on NEXORA. It is a membership platform with packages, referrals and Academy lessons. No guaranteed income — review the packages first: ${link}`)}>Copy</button></div>
     <div className="template"><b>WhatsApp-friendly</b><p>Hi! I use NEXORA for membership + referrals. If you want to see how it works, open this link and we can talk through the packages: {link}</p><button className="secondary" onClick={()=>copyText(`Hi! I use NEXORA for membership + referrals. If you want to see how it works, open this link and we can talk through the packages: ${link}`)}>Copy</button></div>
    </div>
   </div>
   ...
   <div className="panel invitetips">
    ...
    <div className="tipsteps">
     <div className="tipstep"><span>1</span><div><b>Lead with clarity, not pressure</b><p>Explain what NEXORA is: a member workspace with packages, learning, wallet and referral tools. Avoid hype and income promises.</p></div></div>
     <div className="tipstep"><span>2</span><div><b>Show the product first</b><p>Let them open your link, view packages and the public pages. People trust what they can see more than a long pitch.</p></div></div>
     <div className="tipstep"><span>3</span><div><b>Answer the real questions</b><p>Package prices, how upgrades work, how commissions are recorded, and that earnings depend on qualifying purchases — not guarantees.</p></div></div>
     ...
     <div className="tipstep"><span>5</span><div><b>Invite, then follow up once</b><p>Send the link, offer to explain packages, and follow up politely. Do not spam or pressure family and friends.</p></div></div>
   ...
   <div className="panel">
    ...
    <div className="tips">
     ...
     <span><Check size={15}/> Share package details before someone pays.</span>
...
function Academy(){
 ...
 const lessons=[
  ...
  {title:'Growing sustainably & ethically',level:'ADVANCED',time:'8 min',text:'Build a long-term approach based on education, useful communication and respect for other members.',sections:[['Set realistic goals','Focus on learning, consistent communication and quality relationships rather than chasing a promised income target.'],['Be transparent','Explain membership costs, package rules and the fact that commissions depend on qualifying activity.'],['Build relationships','Answer questions honestly, allow people time to decide and direct complex questions to NEXORA Support.']],takeaway:'Sustainable growth comes from informed people, responsible communication and accurate expectations.'}
 ];
...
function PhoneModal({data,onCancel,onContinue,onWallet,onTillInitiate,onTillSubmitCode,walletBalance=0,tillInfo}){
 const [phone,setPhone]=useState(data.phone||"");
 const [err,setErr]=useState("");
 const [method,setMethod]=useState(walletBalance>=Number(data.chargeAmount||0)?"wallet":"till");
 const [busy,setBusy]=useState(false);
 const [walletConfirm,setWalletConfirm]=useState(false);
 const [tillStep,setTillStep]=useState(data.tillSession?"code":"instructions"); // instructions | code | done
 const [tillSession,setTillSession]=useState(data.tillSession||null);
 const [mpesaCode,setMpesaCode]=useState("");
 const amount=Number(data.chargeAmount||data.package?.price||0);
 const canWallet=walletBalance>=amount && amount>0;
 const tillReady=Boolean(tillInfo?.tillNumber||tillSession?.tillNumber);

 const submit=async e=>{
  e.preventDefault();setErr("");
  if(method==="wallet"){
   if(!canWallet) return setErr("Insufficient wallet balance for this package.");
  ...
  // till
  if(tillStep==="instructions"){
   if(!tillReady && !onTillInitiate) return setErr("Till payments are not configured.");
   setBusy(true);
   try{
    const session=await onTillInitiate(data.package);
    setTillSession(session);
    setTillStep("code");
   }catch(ex){ setErr(ex.message||"Could not start till payment"); }
   finally{ setBusy(false); }
   return;
  }
  if(tillStep==="code"){
   ...
   setBusy(true);
   try{
    const r=await onTillSubmitCode(tillSession?.reference, mpesaCode.trim());
    setTillStep("done");
    setErr("");
    setTillSession(s=>({...s, status:r.status, message:r.message}));
 ...
 return <div className="modalbackdrop" onClick={onCancel}><div className="modal phonemodal" onClick={e=>e.stopPropagation()}>
  <div className="modalhead"><div><span className="pill">PURCHASE PACKAGE</span><h2>{tillStep==="done"?"Payment submitted":"Choose how to pay"}</h2></div><button type="button" className="iconbtn" onClick={onCancel} aria-label="Close payment window"><X size={20}/></button></div>
  <form className="phonemodalform" onSubmit={submit}>
   <div className="paymentbody phonebody">
    <div className="phonepackage">
     <div><span>Package</span><strong>{data.package?.name}</strong></div>
    ...
    {tillStep!=="done"&&tillStep!=="code"&&(
     <div className="paymethods">
      <button type="button" className={`paymethod ${method==="till"?"active":""}`} disabled={busy} onClick={async()=>{
       setMethod("till");setErr("");
       if(tillStep!=="instructions" || !onTillInitiate) return;
       if(!tillReady) return setErr("Till payments are not configured yet.");
       setBusy(true);
       try{const session=await onTillInitiate(data.package);setTillSession(session);setTillStep("code");}
       catch(ex){setErr(ex.message||"Could not start till payment");}
       finally{setBusy(false);}
      }}>
       <MessageCircle size={18}/><div><b>{busy&&method==="till"?"Opening M-Pesa payment…":"M-Pesa Till"}</b><span>Tap to continue directly to the till payment details</span></div>
      </button>
    ...
    {method==="wallet"&&tillStep!=="done"&&(
     <div className="walletconfirmbox">
      <p className="muted small">Your wallet will be charged <strong>{money(amount)}</strong>. The package activates immediately. <strong>This cannot be undone.</strong></p>
    ...
    {method==="till"&&tillStep==="instructions"&&(
     <div className="tillbox">
      <p className="muted small">You will pay <strong>{money(amount)}</strong> to the NEXORA M-Pesa till. After paying, you must submit the M-Pesa confirmation code. Your package activates only after we verify the payment amount and code.</p>
      {!tillReady&&<div className="error">Till number is not configured yet. Contact support or use wallet balance.</div>}
     </div>
    )}
    ...
    {method==="till"&&tillStep==="code"&&tillSession&&(
     <div className="tillbox">
      ...
      <ol className="tillsteps">
       ...
       <li>Enter amount <b>{money(tillSession.chargeAmount||amount)}</b> (exact)</li>
       <li>If asked for account/reference, use <b>{displayRef}</b></li>
      ...
      <p className="muted small">We verify the code and that the amount matches before activating your package.</p>
     </div>
    )}
    ...
    {tillStep==="done"&&(
     <div className="repairdone" style={{margin:"12px 0"}}>
      ...
      <div><b>Submitted for verification</b><p>{tillSession?.message||"We will confirm your M-Pesa payment and activate the package once the amount matches."}</p></div>
     </div>
    )}
   ...
   <div className="modalfoot paymentfoot">
    <button type="button" className="secondary" onClick={onCancel} disabled={busy}>{tillStep==="done"?"Close":"Cancel"}</button>
    {tillStep!=="done"&&(
     <button type="submit" className="primary" disabled={busy||(method==="till"&&tillStep==="instructions"&&!tillReady)}>
      {busy?"Please wait…":method==="wallet"?`Pay ${money(amount)} from wallet`:tillStep==="code"?"Submit M-Pesa code":"Continue to till payment"}
     </button>
    )}
   </div>
  </form>
...
function Packages({packages,current,purchase,reload}){
 ...
 <div className="earningsnotice"><div><strong>How package earnings work</strong><p>Starter, Growth, Pro and Elite earn through qualifying referrals. Premium can earn through qualifying referrals across all five package levels and also unlocks the Products & Advertising area for approved campaigns.</p><p>Your package unlocks the package levels you can earn from. Starter can earn from Starter purchases; Growth can earn from Starter + Growth; Pro can earn from Starter + Growth + Pro; Elite can earn from Starter + Growth + Pro + Elite; Premium can earn from all five. When an eligible referral purchases a package, the commission shown for that purchased package applies.</p></div></div>
...
function DepositModal({onClose,load}){
 ...
 return <div className="modalbackdrop" onClick={onClose}><div className="modal depositmodal" onClick={e=>e.stopPropagation()}><div className="modalhead"><div><span className="pill">WALLET DEPOSIT</span><h2>{step==="amount"?"Deposit to wallet":step==="code"?"Complete M-Pesa deposit":"Deposit submitted"}</h2></div><button type="button" className="iconbtn" onClick={onClose}><X size={20}/></button></div>
  <div className="depositbody">
   {step==="amount"&&<form onSubmit={initiate}><p className="muted">Add funds to your NEXORA wallet using the NEXORA M-Pesa Till. Deposits are credited after the payment and confirmation code are verified by NEXORA.</p><label>Deposit amount (KSh)<input required type="number" min="100" step="1" placeholder="e.g. 1,000" value={amount} onChange={e=>setAmount(e.target.value)}/></label>{msg&&<div className="error">{msg}</div>}<button className="primary" disabled={busy}>{busy?"Opening payment…":"Continue to M-Pesa Till"}</button></form>}
   {step==="code"&&session&&<form onSubmit={submit}><div className="tillbox"><div className="tilldetails"><div className="tillrowcopy"><div><span>Till / Buy Goods</span><strong className="tillnumber">{session.tillNumber}</strong></div><button type="button" className="secondary copytillbtn" onClick={()=>copy(session.tillNumber,"till")}>{copied==="till"?<><Check size={14}/> Copied</>:<><Copy size={14}/> Copy till</>}</button></div><div><span>Business name</span><strong>{session.tillName||"NEXORA"}</strong></div><div className="tillrowcopy"><div><span>Amount</span><strong>{money(session.amount)}</strong></div><button type="button" className="secondary copytillbtn" onClick={()=>copy(session.amount,"amount")}>{copied==="amount"?<><Check size={14}/> Copied</>:<><Copy size={14}/> Copy amount</>}</button></div><div className="tillrowcopy"><div><span>Reference</span><strong className="refcode">{session.reference}</strong></div><button type="button" className="secondary copytillbtn" onClick={()=>copy(session.reference,"ref")}>{copied==="ref"?<><Check size={14}/> Copied</>:<><Copy size={14}/> Copy ref</>}</button></div></div><ol className="tillsteps"><li>Open M-Pesa → Lipa na M-Pesa → Buy Goods and Services.</li><li>Enter Till <b>{session.tillNumber}</b> and amount <b>{money(session.amount)}</b>.</li><li>If asked for a reference, use <b>{session.reference}</b>.</li><li>Complete the payment and keep the confirmation SMS.</li></ol><label className="fieldlabel">M-Pesa confirmation code<input required value={code} onChange={e=>setCode(e.target.value.toUpperCase())} placeholder="e.g. QH12ABCDE1" autoComplete="off"/></label>{msg&&<div className="error">{msg}</div>}<button className="primary" disabled={busy}>{busy?"Submitting…":"Submit confirmation code"}</button></div></form>}
