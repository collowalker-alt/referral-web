  ...
  const pages={
    terms:{
      ...
      sections:[
        ...
        ["Membership & payments","Plan purchases and upgrades are processed through the payment providers configured by NEXORA (including Co-op Bank M-Pesa). Charges, upgrades (price difference only) and refunds follow the rules shown in the platform at the time of the transaction."],
    ...
    privacy:{
      ...
      sections:[
        ...
        ["Payments","Payment processing is handled by Co-operative Bank M-Pesa STK Push. NEXORA does not store full card or M-Pesa PIN data."],
...
function PaymentModal({payment,onClose,onCheck}){
 ...
 return <div className="modalbackdrop" onClick={onClose}><div className="modal paymentmodal" onClick={e=>e.stopPropagation()}><div className="modalhead"><div><span className={`pill ${isSuccess?"payment-success":isFailed?"payment-failed":"payment-pending"}`}>{isSuccess?"COMPLETED":isFailed?"FAILED":"WAITING FOR PAYMENT"}</span><h2>{title}</h2></div><button className="iconbtn" onClick={onClose}><X size={20}/></button></div><div className="paymentbody"><div className="paymentstatus"><div className={`paymenticon ${isSuccess?"success":isFailed?"failed":"pending"}`}>{isSuccess?<CheckCircle2 size={28}/>:isFailed?<X size={28}/>:<RefreshCw size={28}/>}</div><div><b>{text}</b><p className="muted">{isSuccess?"Payment confirmed by NEXORA.":isFailed?(payment.display_text||"Please try the payment again."):(payment.display_text||"Please enter your M-Pesa PIN on your phone to approve the payment.")}</p></div></div><div className="paymentdetails"><div><span>Plan</span><strong>{payment.package?.name}</strong></div><div><span>Amount</span><strong>{money(payment.chargeAmount||payment.package?.price)}</strong></div><div><span>M-Pesa number</span><strong>{payment.phone}</strong></div><div><span>Reference</span><strong>{payment.reference}</strong></div></div>{isPending&&<div className="paymentwait"><div className="paymentwaittitle"><span className="live-dot"></span><strong>Waiting for confirmation</strong></div><p className="muted small">We will keep checking automatically. Co-op Bank uses a status-query flow; NEXORA checks the transaction automatically until a final status is returned.</p></div>}{payment.message&&<p className="muted small">{payment.message}</p>}</div><div className="modalfoot paymentfoot">{isPending&&<button className="secondary" onClick={onCheck}><RefreshCw size={16}/> Check payment status</button>}<button className="primary paymentclose" onClick={onClose}>{isSuccess?"Go to dashboard":isFailed?"Close":"Close for now"}</button></div></div></div>}
