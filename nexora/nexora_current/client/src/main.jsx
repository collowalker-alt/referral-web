...
function AdminUsers({rows,search,setSearch,action}){
 ...
 const changePlan=async actionName=>{if(!planAction)return;const u=planAction.user;const label=actionName==="DEACTIVATE"?"deactivate":"remove";const ok=await action(()=>adminApi(`/admin/users/${u.id}/plan`,{method:"PATCH",body:JSON.stringify({action:actionName})}),`${u.package?.name||"Plan"} ${label}d for ${u.email}`);if(ok){setPlanAction(null);setDetail(null)}};
 return <section className="adminsection">
  ...
  {planAction&&<div className="modalbackdrop" onClick={()=>setPlanAction(null)}><div className="modal adminbalance" onClick={e=>e.stopPropagation()}><div className="modalhead"><div><span className="pill">PLAN CONTROL</span><h2>Manage member plan</h2><p className="muted">{planAction.user.name} · {planAction.user.email}</p></div><button className="iconbtn" onClick={()=>setPlanAction(null)}><X size={20}/></button></div><div className="adminplanconfirm"><div className="adminplansummary"><span>Current plan</span><strong>{planAction.user.package?.name||"No plan"}</strong><small>{money(planAction.user.package?.price||0)} · assigned to this member</small></div><p className="muted">Choose how to change this member's plan. Their historical transactions remain available in the audit trail.</p><div className="adminplanoptions"><button className="rowaction dangeraction" onClick={()=>changePlan("DEACTIVATE")}><Ban size={15}/> Deactivate plan</button><button className="rowaction dangeraction" onClick={()=>changePlan("REMOVE")}><Trash2 size={15}/> Remove plan</button><button className="secondary" onClick={()=>setPlanAction(null)}>Cancel</button></div></div></div></div>}
