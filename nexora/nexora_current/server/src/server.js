...
app.patch("/api/admin/users/:id/plan",adminAuth,async(req,res)=>{
  try{
    ...
    if(!["DEACTIVATE","REMOVE"].includes(action)) return res.status(400).json({message:"Plan action must be DEACTIVATE or REMOVE"});
    const current=await prisma.user.findUnique({where:{id:req.params.id},include:{package:true}});
    if(!current) return res.status(404).json({message:"User not found"});
    if(!current.packageId) return res.status(400).json({message:"This user does not have an active plan"});
    const previousPackage=current.package;
    const updated=await prisma.user.update({where:{id:current.id},data:{packageId:null},include:{package:true}});
    await logAdminAction(req,action==="DEACTIVATE"?"USER_PLAN_DEACTIVATED":"USER_PLAN_REMOVED","USER",current.id,current.email,{previousPackageId:previousPackage.id,previousPackageName:previousPackage.name,action});
    await createUserNotification(current.id,{title:action==="DEACTIVATE"?"Your membership plan was deactivated":"Your membership plan was removed",message:`An administrator ${action==="DEACTIVATE"?"deactivated":"removed"} your ${previousPackage.name} plan. Your historical payment records remain available in Transactions.`,type:"MEMBERSHIP",details:{action,previousPackageId:previousPackage.id,previousPackageName:previousPackage.name}});
    res.json({message:`${previousPackage.name} plan ${action==="DEACTIVATE"?"deactivated":"removed"} for ${current.email}`,user:{id:updated.id,package:null},previousPackage:{id:previousPackage.id,name:previousPackage.name}});
