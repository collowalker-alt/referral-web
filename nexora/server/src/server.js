...
app.patch("/api/admin/users/:id/plan",adminAuth,async(req,res)=>{
  try{
    ...
    if(action==="REACTIVATE" && !["SUSPENDED","DEACTIVATED"].includes(current.planStatus)) return res.status(400).json({message:"This plan is already active"});
